// Cylindrical.cpp: Implementation of the Cylindrical class.
...
Point::Point()  : Set::Array(3) {}

Point::Point(double * const &u) : Set::Array(3,u) {}
...
Vector::Vector() : Set::VectorSpace::Vector(3) {}

Vector::Vector(double * const &u) : Set::VectorSpace::Vector(3,u) {}
...
Vector::Vector(const Set::Euclidean::Cylindrical::Point &A) : 
	Set::VectorSpace::Vector(A) {}
...
Vector & 
Vector::operator = (const Set::Euclidean::Cylindrical::Point &A)
{
	Set::VectorSpace::Vector::operator = (A);
	return *this;
}
...
Covector::Covector() : 
	Set::VectorSpace::Vector(3) {}

Covector::Covector(double * const &u) : 
	Set::VectorSpace::Vector(3,u) {}
...
Covector::Covector(const Set::Euclidean::Cylindrical::Vector &A) : 
	Set::VectorSpace::Vector(A) {}
...
CovectorZero::CovectorZero(const Set::Euclidean::Cylindrical::VectorZero &A) : 
	Set::VectorSpace::VectorZero(A) {}
...
Embedding<0>::operator () (const Set::Euclidean::Cylindrical::Point &x)
{
	y[0] = x[0]*cos(x[1]); 
	y[1] = x[0]*sin(x[1]);
	y[2] = x[2]; return y;
...
Embedding<1>::Embedding() : A(3) {}

Embedding<1>::Embedding(const Embedding<0> &f) : A(3) {}
...
Embedding<1>::operator () (const Set::Euclidean::Cylindrical::Point &x)
{
	Null(A);
	double c = cos(x[1]); 
	double s = sin(x[1]); 
...
Embedding<2>::Embedding() : A(9,3) {}

Embedding<2>::Embedding(const Embedding<1> &f) : A(9,3) {}
...
const Set::VectorSpace::Hom &
Embedding<2>::operator () (const Set::Euclidean::Cylindrical::Point &x)
{
	Null(A);
	double c = cos(x[1]); 
	double s = sin(x[1]); 
	Set::VectorSpace::Hom A0(3,A[0].begin());
	Set::VectorSpace::Hom A1(3,A[1].begin());
	A0[1][0] =      - s; 
	A0[1][1] =        c;
	A1[0][0] =      - s; 
	A1[0][1] =        c;
...
Submersion<1>::Submersion() : A(3) {}

Submersion<1>::Submersion(const Submersion<0> &f) : A(3) {}
...
Submersion<2>::Submersion() : A(9,3) {}

Submersion<2>::Submersion(const Submersion<1> &f) : A(9,3) {}
...
operator + (const Set::Euclidean::Cylindrical::Point &P,
			const Set::Euclidean::Cylindrical::Vector &A)
{
	Set::Euclidean::Cylindrical::Point Q = P; 
	Q += A; return Q;
}
...
operator - (const Set::Euclidean::Cylindrical::Point &P1,
			const Set::Euclidean::Cylindrical::Point &P2)
{
	assert(P1.size() == 3); 
	assert(P2.size() == 3); 
	Set::Euclidean::Cylindrical::Vector A;
	double &dr = *A.begin();  
	double &dt = *(A.begin()+1);
	double &dz = *(A.begin()+2);
	double &r1 = *P1.begin(); 
	double &t1 = *(P1.begin()+1);
	double &z1 = *(P1.begin()+2);
	double &r2 = *P2.begin(); 
	double &t2 = *(P2.begin()+1);
	double &z2 = *(P2.begin()+2);
...
void Random(Set::Euclidean::Cylindrical::Point &P){P.Randomize();}
