...
__global__
void
setNegativePatchesKernel(const std::size_t size, const uint3* indices,
                         const uint3 extents, unsigned char* sign) {
   // Convert the 2-D block index into a single array index.
   const std::size_t arrayIndex = blockIdx.x + blockIdx.y * gridDim.x;
   if (arrayIndex >= size) {
      return;
   }
...
__global__
void
setSignKernel(const std::size_t numRefined,
              const float* patches, const uint3* indices,
              const uint3 extents, unsigned char* sign) {
   // Convert the 2-D block index into a single patch index.
   const std::size_t patchIndex = blockIdx.x + blockIdx.y * gridDim.x;
   if (patchIndex >= numRefined) {
      return;
   }
   // Select a single patch using the block index.
   const std::size_t PatchSize = PatchExtent * PatchExtent * PatchExtent;
   ...
   unsigned char s = 0;
   const std::size_t offset = threadIdx.y * PatchExtent +
      threadIdx.z * PatchExtent * PatchExtent;
   for (std::size_t i = PatchExtent; i-- != 0; ) {
      s <<= 1;
      s |= patch[offset + i] > 0;
   }
   sign[index.x + (index.y * PatchExtent + threadIdx.y) * extents.x +
...
__global__
void
markOutsideKernel(const std::size_t numRefined,
   ...
   const std::size_t offset = threadIdx.y * PatchExtent +
      threadIdx.z * PatchExtent * PatchExtent;
   for (std::size_t i = 0; i != PatchExtent; ++i) {
      if (mask & out) {
...
(const std::tr1::array<std::size_t, 3>& gridExtents,
 ...
 std::vector<bool>* outsideAtLowerCorners) {
   ...
   // the bit arrays may be larger that the grid in the x direction. Any 
