// Polytropic.cpp: implementation for the Polytropic class.
// Copyright (c) 2006 by Bo Li-All rights reserved
//////////////////////////////////////////////////////////////////////
...
Energy<0>::operator () (const double &J) const
{
	...
	double G = Gamma-1.0;
	return (Rho0*C/G)*pow(Rho0/fabs(J),G);
}
...
Energy<1>::operator () (const double &J) const
{
	...
	double p = -C*pow(Rho0/fabs(J),Gamma);        
...
Jet<0>::operator () (const double &J) const 
{
	...
	double G = Gamma-1.0;
	double Rho = Rho0/J;
	double W = (Rho0*C/G)*pow(Rho,G);
	double DW = -C*pow(Rho,Gamma);
...
Jet<1>::operator () (const double &J) const
{
    ...
    double Rho = Rho0/J;
    double DW = -C*pow(Rho,Gamma);
    double DDW = (C*Gamma/Rho0)*pow(Rho,Gamma+1.0); 
