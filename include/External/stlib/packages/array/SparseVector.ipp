...
#if !defined(__array_SparseVector_ipp__)
...
operator+=(vector<_T1>& x, const array::SparseVector<_T2>& y) {
   for (typename array::SparseVector<_T2>::const_iterator i = y.begin();
         i != y.end(); ++i) {
      x[i->first] += i->second;
   }
   return x;
...
scaleAdd(vector<_T1>* x, const _T2 a, const array::SparseVector<_T3>& y) {
   for (typename array::SparseVector<_T3>::const_iterator i = y.begin();
         i != y.end(); ++i) {
